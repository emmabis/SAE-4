...
<div id="toutdashboard">
    
<div class="station-info-container">
    <h2 class="station-info-title">Informations sur la station</h2>
    <?php

    
    // Vérifier si la station est définie
    if (isset($stations)) {
        // Récupérer l'ID de la station depuis l'URL
        $stationId = htmlspecialchars($_GET['station_id'] ?? '');

        // Trouver la station correspondante
        $station = null;
        foreach ($stations as $s) {
            if ($s->getId_station() === $stationId) {
                $station = $s;
                break;
            }
        }
        ...
        if ($station) {
            // Construire les données de la station
            echo "<ul class='station-info-list'>";
            echo "<li><span class='info-label'>Nom de la station:</span> <span class='info-value'>" . htmlspecialchars($station->getNom()) . "</span></li>";
            echo "<li><span class='info-label'>Code Station:</span> <span class='info-value'>" . htmlspecialchars($station->getId_station()) . "</span></li>";
            echo "<li><span class='info-label'>Altitude:</span> <span class='info-value'>" . htmlspecialchars($station->getAltitude()) . " mètres</span></li>";
            echo "<li><span class='info-label'>Latitude:</span> <span class='info-value'>" . htmlspecialchars($station->getLatitude()) . "°</span></li>";
            echo "<li><span class='info-label'>Longitude:</span> <span class='info-value'>" . htmlspecialchars($station->getLongitude()) . "°</span></li>";
            echo "<li><span class='info-label'>Région:</span> <span class='info-value'>" . htmlspecialchars($station->getNom_reg()) . "</span></li>";
            echo "<li><span class='info-label'>Département:</span> <span class='info-value'>" . htmlspecialchars($station->getNom_dept()) . " (Code postal: " . htmlspecialchars($station->getCode_dept()) . ")</span></li>";
            echo "<li><span class='info-label'>EPCI:</span> <span class='info-value'>" . htmlspecialchars($station->getNom_epci()) . " (Code EPCI: " . htmlspecialchars($station->getCode_epci()) . ")</span></li>";
            echo "<li><span class='info-label'>Code géographique:</span> <span class='info-value'>" . htmlspecialchars($station->getCode_geo()) . "</span></li>";
if (isset($_GET['date'])) {
    $d = $_GET['date'];
    echo "<li><span class='info-label'>Date choisit:</span> $d </li>";
}
            echo "</ul>";
        } else {
            echo "<div class='station-error'>⚠️ Aucune station trouvée avec l'ID : $stationId</div>";
        }
    } else {
        echo "<div class='station-error'>⚠️ Les données des stations ne sont pas disponibles.</div>";
    }
    ?>
</div>
...
$values2 = [];
    ...
    // Tableau de conversion des jours en français
$joursEnFrancais = [
    "Monday"    => "Lundi",
    "Tuesday"   => "Mardi",
    "Wednesday" => "Mercredi",
...
$labels2 = []; // Initialisation

for ($i = 0; $i < 7; $i++) {
    $dateStr = $stationpardate[$i][1]; // Récupère la date
    $jourEnAnglais = (new \DateTime($dateStr))->format('l'); // Jour en anglais
    $jourEnFrancais = $joursEnFrancais[$jourEnAnglais]; // Conversion en français

    $labels2[] = $jourEnFrancais; // Stocke le jour en français

    if (isset($stationpardate[$i][0])) {
        $values2[] = $stationpardate[$i][0]->getPression() /100 ?? 0;
    } else {
        $values2[] = 0;
    }
}
?>
    <script>
        ...
        const data2 = <?php echo json_encode($values2); ?>;
            ...
            options: {
                ...
                scales: {
                    ...
                    y: {
                        ...
                        min: Math.min(...data2) - 10,
                        max: Math.max(...data2) + 2,
                        beginAtZero: false
                    }
                }
            }
        ...
        new Chart(document.getElementById('humidityChart2'), config2);
    </script>
</div>

<div class="graphes">
    <canvas id="getTemperature"></canvas>
    <div class="graphes">
    <canvas id="getTemperature"></canvas>
    <?php
$labels3 = [];
...
for ($i = 0; $i < 7; $i++) {
    ...
    $labels3[] = $jourEnFrancais; // Stocke le jour en français

    
    if(isset($stationpardate[$i+1][0])){
        $values3[] = $stationpardate[$i+1][0]->getTemperature() ?? 0;
    } else {
        $values3[] = 0;
    }
}
?>
    ...
    <script>
        const labels3 = <?php echo json_encode($labels3); ?>;
        const data3 = <?php echo json_encode($values3); ?>;
        ...
        const config3 = {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: data3,
                    fill: false,
                    borderColor: 'rgb(173, 19, 19)',
                    tension: 0.0,
                    pointBackgroundColor: 'rgb(173, 19, 19)',
                    pointBorderColor: '#000',
                    pointRadius: 4
                }]
            },
            options: {
                ...
                scales: {
                    ...
                    y: {
                        ...
                        min: Math.min(...data3) - 5,
                        max: Math.max(...data3) + 5
                    }
                }
            }
        };

        new Chart(document.getElementById('getTemperature'), config3);
    </script>
</div>
...
<div class="forecast-block">
        <h2 class="forecast-title">Type de nuage</h2>
...
for ($i = 0; $i < 7; $i++) {
    ...
    $labels2[] = $jourEnFrancais; // Stocke le jour en français
        ...
        if (isset($stationpardate[$i][0])) {
            $forecastValue = $stationpardate[$i][0]->getTypeNuage() ?? "No data"; // Use "No data" as default
        } else {
            $forecastValue = "No data";
        }
        ?>
      <li class="forecast-item">
    <?php echo $jourEnFrancais; ?> : 
    <?php if ($forecastValue === "Aucun nuage détecté"): ?>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#f39c12" style="width: 1.5rem; height: 1rem;">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-4.227-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z" />
        </svg>
    <?php elseif ($forecastValue === "No data"): ?>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#6b7280" style="width: 1.5rem; height: 1rem;">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 3.75h.008v.008H12v-.008Z" />
        </svg>
    <?php else: ?>
        <span class="forecast-value"><?php echo $forecastValue; ?></span>
    <?php endif; ?>
</li>
...
<?php
    ...
    for ($i = 7; $i >= 1; $i--) {
        if (isset($stationpardate[$i][0])) {
            $values5 += $stationpardate[$i][0]->getHumidite() ?? 0;
        } else {
            $values5 += 0;
        }
    }
    $values5 = $values5 / 7; // Moyenne calculée
?>
<script>
// Récupération des données PHP
const singleValue = <?php echo json_encode($values5); ?>;

// Configuration de la jauge
const gaugeConfig = {
    type: 'doughnut', // Utilisation du type doughnut pour simuler une jauge
    data: {
        labels: ['Humidité (%)', ''],
        datasets: [{
            data: [singleValue, 100 - singleValue], // Valeur et complément
            backgroundColor: ['rgb(173, 88, 19)', 'rgb(230, 230, 230)'],
            hoverBackgroundColor: ['rgb(139, 70, 15)', 'rgb(200, 200, 200)'],
            borderWidth: 0
        }]
    },
    options: {
        responsive: true,
        rotation: -90, // Rotation de départ (-90°)
        circumference: 180, // Affiche uniquement une demi-jauge (180°)
        cutout: '75%', // Taille intérieure pour l’effet de jauge
        plugins: {
            title: {
                display: true, // Active le titre
                text: 'Taux d\'Humidité Moyen', // Texte du titre
                color: 'white',
                font: {
                    size: 18,
                    weight: 'bold'
                ...
                padding: {
                    top: 10, // Espace au-dessus du titre
                    bottom: 5 // Espace en-dessous du titre
                }
            },
            legend: {
                display: true // Masque la légende pour une jauge épurée
            },
            tooltip: {
                callbacks: {
                    label: function(context) {
                        return context.label + ': ' + context.raw + '%'; // Infobulles personnalisées
                    }
                }
            }
        }
...
const ctx = document.getElementById('singleValueChart').getContext('2d');
new Chart(ctx, gaugeConfig);
</script>



<div class="forecast-block" id="nebulosite">
    <h2 class="forecast-title">Type de nébulosité</h2>
    <ul class="forecast-list">
    <?php
    ...
    // Date de départ (premier jour)
    $dateDepart = isset($stationpardate[1][1]) ? strtotime($stationpardate[1][1]) : time();

    for ($i = 0; $i <= 7; $i++) {
        // Initialize default values
        $jourEnAnglais = "Unknown";
        $forecastValue = "données indisponibles";

        // Calcul de la date pour chaque jour à partir de la date de départ
        $timestamp = strtotime("+" . ($i-1) . " days", $dateDepart);
        $jourEnAnglais = date("l", $timestamp);
        $jourEnFrancais = $joursEnFrancais[$jourEnAnglais];

        // Check if the object exists and get the value
        if (isset($stationpardate[$i][0])) {
            $forecastValue = $stationpardate[$i][0]->getnebulositetotal() ?? "données indisponibles";
        }
        ?>
        <li class="forecast-item">
            <?php echo $jourEnFrancais; ?> : <span class="forecast-value"><?php echo $forecastValue; ?> </span>
        </li>
    <?php } ?>
</ul>
...
<div class="dg">
<div class="graphes" id="vitessevent">
    <canvas id="VitesseVentChart"></canvas>
    <?php
$labels5 = [];
$values5 = [];

// On récupère la date de départ à partir du premier élément
// Tableau de conversion des jours en français
$joursEnFrancais = [
    "Monday"    => "Lundi",
    "Tuesday"   => "Mardi",
...
for ($i = 0; $i < 7; $i++) {
    ...
    $labels5[] = $jourEnFrancais; // Stocke le jour en français
    
    if(isset($stationpardate[$i+1][0])){
        $values5[] = $stationpardate[$i+1][0]->getVitesseVent() ?? 0;
    } else {
        $values5[] = 0;
    }
}
?>
    <script>
        const labels5 = <?php echo json_encode($labels5); ?>;
        const data5 = <?php echo json_encode($values5); ?>;
        ...
        const config5 = {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: data5,
                    fill: false,
                    borderColor: 'rgb(140, 39, 160)',
                    tension: 0.0,
                    pointBackgroundColor: 'rgb(140, 39, 160)',
                    pointBorderColor: '#000',
                    pointRadius: 4
                }]
            },
            options: {
                ...
                scales: {
                    ...
                    y: {
                        ...
                        min: Math.min(...data5) - 1,
                        max: Math.max(...data5) + 1,
                        stepSize: 1,
                        beginAtZero: false
                    }
                }
        ...
        const myChart5 = new Chart(
            document.getElementById('VitesseVentChart'),
            config5
        );
    </script>
</div>
<div class="graphes">
<div class="type">
<div class='lesdeuxparagraphes'>
<div class="forecast-blocks">
    <h2 class="forecast-title">Portée visuelle</h2>
    <ul class="forecast-list">
    <?php
$joursEnFrancais = [
    "Monday"    => "Lundi",
...
for ($i = 0; $i < 7; $i++) {
    ...
    $labels6[] = $jourEnFrancais; // Stocke le jour en français
    

        $visibilite = isset($stationpardate[$i][0]) ? ($stationpardate[$i][0]->getvisibilitehorizontale() ?? 0) : 0;
        ?>
    <li class="forecast-item">
        <?php echo $jourEnFrancais; ?> :
        <span class="forecast-value">
            <?php echo ($visibilite != 0) ? $visibilite . ' m' : 'données indisponibles'; ?>
        </span>
    </li>
    <?php } ?>
</ul>
...
<div class="forecast-blocks">
    <h2 class="forecast-title">Tendance de la pression atmosphérique</h2>
    <ul class="forecast-list">
    <?php
    // Tableau de traduction des jours
    $joursEnFrancais = array(
    ...
    $dateStr = $stationpardate[0][1]; // Récupère la date
    $jourEnAnglais = (new \DateTime($dateStr))->format('l'); // Jour en anglais
    $jourEnFrancais = $joursEnFrancais[$jourEnAnglais]; 

        // Récupération de la tendance barométrique
        $tendanceBarometrique = isset($stationpardate[$i][0]) ? ($stationpardate[$i][0]->gettypetendancebarometrique() ?? 'no data') : 0;
    ?>
        <li class="forecast-item">
            <?php echo $jourEnFrancais; ?> :
            <span class="forecast-value"><?php echo $tendanceBarometrique; ?></span>
        </li>
...
<form method="GET" action="" class="date-form">
    <input type="hidden" name="action" value="readStationDash">
    <input type="hidden" name="controller" value="station">
    <input type="hidden" name="station_id" value="<?php echo htmlspecialchars($_GET['station_id']); ?>">

    <label for="dateInput">Choisir une date :</label>
    <input type="date" id="dateInput" name="date" max="" required>

    <button type="submit">Envoyer</button>
</form>

<script>
    // Fonction qui définit la date max comme étant la date actuelle moins un jour
    function setMaxDate() {
        // Créer un objet Date pour aujourd'hui
        const today = new Date();

        // Soustraire 1 jour (24 heures)
        today.setDate(today.getDate() - 1);

        // Formater la date au format YYYY-MM-DD (compatible avec input[type="date"])
        const maxDate = today.toISOString().split('T')[0];

        // Appliquer la date max à l'input
...
<div id="cartedash">
    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" x="0" y="0" viewBox="0 -14 844 1198" xml:space="preserve">
        <a  xlink:title="CORSE"   href="frontController.php?action=regionCarte&controller=station&region=Corse">
      ...
      <a  xlink:title="CENTRE-VAL DE LOIRE"   href="frontController.php?action=regionCarte&controller=station&region=Centre-Val de Loire">  
      ...
      <a  xlink:title="BRETAGNE"   href="frontController.php?action=regionCarte&controller=station&region=Bretagne">
      ...
      <a  xlink:title="OCCITANIE"   href="frontController.php?action=regionCarte&controller=station&region=Occitanie">
      ...
      <a  xlink:title="HAUTS-DE-FRANCE"   href="frontController.php?action=regionCarte&controller=station&region=Hauts-de-France">
      ...
      <a  xlink:title="PAYS DE LA LOIRE"   href="frontController.php?action=regionCarte&controller=station&region=Pays de la Loire">
      ...
      <a  xlink:title="GUYANE"   href="frontController.php?action=regionCarte&controller=station&region=Guyane">
      ...
      <a  xlink:title="MARTINIQUE"   href="frontController.php?action=regionCarte&controller=station&region=Martinique">
      ...
      <a  xlink:title="MAYOTTE"   href="frontController.php?action=regionCarte&controller=station&region=Mayotte">
      ...
      <a  xlink:title="GUADELOUPE"   href="frontController.php?action=regionCarte&controller=station&region=Guadeloupe">
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (region) {
        let nv = document.querySelector('[id="' + region + '"]'); 
        if (nv) {
            nv.style.fill = "#D32F2F";
        } else {
            console.error("Élément introuvable !");
        }
    } else {
        console.error("Paramètre 'region' non trouvé dans l'URL !");
    }
});
